...
import { mainTabToPath, routeToMainTab, type MainTab } from './lib/appRouting';
import {
...
const TaskCenter = React.lazy(() =>
  import('./components/TaskCenter').then((module) => ({ default: module.TaskCenter }))
);
...
function LazySectionFallback({ label }: { label: string }) {
  return (
    <div className="max-w-[1500px] mx-auto pb-12">
      <div className="border border-white/10 bg-white/[0.03] rounded-[28px] p-8 text-sm text-white/50">
        Loading {label}...
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [openRouterKey, setOpenRouterKey] = useState(
    () => localStorage.getItem('openRouterKey') || (process.env.OPENROUTER_KEY || '').split(',')[0].trim() || ''
  );
  const [openRouterModel, setOpenRouterModel] = useState(
    () => localStorage.getItem('openRouterModel') || process.env.OPENROUTER_MODEL || 'qwen/qwen3-30b-a3b:free,qwen/qwen3-next-80b-a3b-instruct:free,meta-llama/llama-3.1-8b-instruct:free'
  );
            ...
            while (!aiSuccess && retries < MAX_RETRIES) {
              try {
                ...
                if (aiProvider === 'local') {
                  ...
                  const orModels = openRouterModel.split(',').map((m: string) => m.trim()).filter(Boolean);
                  let orLastErr = '';
                  let orDone = false;
                  for (let mi = 0; mi < orModels.length; mi++) {
                    const model = orModels[mi];
                    ...
                    if (!res.ok) {
                      const errText = await res.text().catch(() => '');
                      orLastErr = `OpenRouter Error ${res.status}: ${errText}`;
                      if (res.status === 429 && mi < orModels.length - 1) {
                        setPipelineLogs(prev => [...prev, `⚠️ Model "${model}" rate-limited (429), trying next fallback...`]);
                        continue;
                      }
                      throw new Error(orLastErr);
                    }
                    const data = await res.json();
                  ...
                  if (!orDone) {
                    throw new Error(orLastErr || 'All OpenRouter models rate-limited');
                  }
                } else {
                ...
                if ((aiProvider === 'gemini' && (errMsg.toLowerCase().includes('quota') || is429)) ||
                    (aiProvider === 'openrouter' && is429)) {
                  retries++;
                  const waitSec = aiProvider === 'openrouter' ? 15 * retries : 10 * retries;
                  setPipelineLogs(prev => [...prev, `⚠️ All models rate-limited (attempt ${retries}/${MAX_RETRIES}). Waiting ${waitSec}s...`]);
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 custom-scrollbar bg-[#0b0601]">
          <AnimatePresence mode="wait">
                        ...
                        <div className="space-y-6 max-h-[70vh] overflow-y-auto px-1 custom-scrollbar">
                          ...
                          {aiProvider === 'local' ? (
                            ...
                            <div className="space-y-4 pt-2">
                              ...
                              <div className="space-y-1.5">
                                <label className="text-[10px] text-[#4F46E5] uppercase pl-1">Models — comma-separated fallback list</label>
                                <textarea
                                  rows={3}
                                  value={openRouterModel}
                                  onChange={(e) => setOpenRouterModel(e.target.value)}
                                  className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-xs focus:border-[#4F46E5]/50 transition-all focus:outline-none resize-none"
                                  placeholder="qwen/qwen3-30b-a3b:free,meta-llama/llama-3.1-8b-instruct:free"
                                />
                                <p className="text-[9px] text-white/30 pl-1">Tried left-to-right; switches automatically on 429 rate-limit.</p>
                              </div>
                            </div>
                          ) : (
            ...
            {activeMainTab === 'taskcenter' && (
              ...
              >
                <React.Suspense fallback={<LazySectionFallback label="Task Center" />}>
                  <TaskCenter
